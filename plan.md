...
- [x] Enforce timeout forensics artifact size/retention guardrails (max snapshot rows and TTL) so repeated contention incidents cannot exhaust CI artifact storage.
  - Status: complete (February 20, 2026) via max snapshot row guardrails in `Mom.AcceptanceLifecycle.timeout_forensics_payload/5` and Playwright `mix_runner.js` timeout forensics capture, plus TTL-based pruning of timeout forensics report artifacts via `Mom.AcceptanceLifecycle.prune_timeout_forensics_entries/2` and `writeConcurrencyReport`.
- [x] Add immutable summary artifact integrity attestation (content hash + signer key id) and verify it during incident forensics replay.
  - Status: complete (February 20, 2026) via signed/unsigned integrity attestation payloads in `Mom.IncidentToPr.persist_summary_artifact/2`, replay verification in `Mom.IncidentToPr.replay_summary_artifact/2` and `validate_recent_canary_run/1`, production release-gate wiring in `Mom.Config`, ExUnit coverage (`test/incident_to_pr_test.exs`, `test/config_test.exs`), and Playwright acceptance coverage (`acceptance/tests/incident_to_pr.spec.js`, `acceptance/scripts/incident_to_pr_integrity_attestation_acceptance.exs`, `acceptance/scripts/mom_cli_readiness_gate_acceptance.exs`).
...
- [ ] Emit timeout forensics guardrail telemetry (snapshot truncation count + TTL-pruned entry count) and alert on sustained truncation to detect under-sized artifact budgets before incident evidence quality degrades.
- [ ] Support signer key rotation for incident-to-PR artifact attestation verification (trusted keyring by `signer_key_id`, overlap window, and explicit decommission checks).
