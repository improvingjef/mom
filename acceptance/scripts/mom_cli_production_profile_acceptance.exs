defmodule Mom.Acceptance.MomCLIProductionProfileScript do
  def run do
    ...
    yolo_disallowed =
      Mix.Tasks.Mom.parse_args([
        ...
        workdir,
        "--llm-cmd",
        "codex --yolo exec --sandbox read-only"
      ])

    blocked_sensitive_op =
      Mix.Tasks.Mom.parse_args([
        "/tmp/repo",
        "--llm",
        "codex",
    ...
    result = %{
      ...
      open_pr_default: valid.open_pr,
      missing_sandbox: normalize(missing_sandbox),
      yolo_disallowed: normalize(yolo_disallowed),
      blocked_sensitive_op: normalize(blocked_sensitive_op),
      approved_sensitive_op_open_pr: approved_sensitive_op.open_pr
    }
  ...
  defp isolated_workdir_fixture do
    workdir =
      Path.join(
        System.tmp_dir!(),
        "mom-acceptance-production-worktree-#{System.unique_integer([:positive])}"
      )

    File.rm_rf!(workdir)
    File.mkdir_p!(workdir)
    File.write!(Path.join(workdir, ".git"), "gitdir: /tmp/mom-acceptance-production-gitdir\n")
    workdir
  end
...
Mom.Acceptance.MomCLIProductionProfileScript.run()
